    ...
    try {
      ...
      for (const item of result.data.data) {
        ...
        const lignes = [
          ...
          ...(item.proforma?.ligneprestationarticle || [])
        ];
        
        const processedLignes = concatenateQueryset(lignes);
        const finalLignes = handleEnterposage(processedLignes);

        const reference = item.facture_number ? 
        ...
        const _411102 = {
          ...
          "REFERENCE": reference,
          "LIBELLE": `${item.proforma?.article?.client?.raison_sociale} G-${item.proforma?.gros?.numero} A-${item.proforma?.article?.numero}`  || "",
          "REBRIQUE": "",
          "REGEME": item.proforma?.gros?.regime?.designation || "",
          "CODE_JRN": "",
          "CODE_COM": "411102",
          "CODE_AUX": "",
          "CODE_BDG": "",
        ...
        for (const ligne of finalLignes) {
          const row = {
            "": "",
            "PIECE": 0,
            "DATE": item.proforma?.date_proforma || "",
